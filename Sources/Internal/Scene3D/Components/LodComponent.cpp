...
{
    return std::sqrt(nearDistanceSq);
}
            ...
            if (NULL != lodDataArch)
            {
                ...
                for (uint32 i = 0; i < lodDataCount; ++i)
                {
                    ...
                    if (NULL != lodDataValuesArch)
                    {
                        ...
                        if (lodDataValuesArch->IsKeyExists("layer"))
                            data.layer = lodDataValuesArch->GetInt32("layer");
                        if (lodDataValuesArch->IsKeyExists("isdummy"))
                            data.isDummy = lodDataValuesArch->GetBool("isdummy");
