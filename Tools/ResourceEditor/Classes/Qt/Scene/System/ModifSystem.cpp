...
void EntityModificationSystem::ResetTransform(const SelectableGroup& entities)
{
    ...
    if (nullptr != sceneEditor && ModifCanStart(entities))
    {
        ...
        {
            sceneEditor->Exec(DAVA::Command::Create<TransformCommand>(item, item.GetLocalTransform(), zeroTransform));
        }
...
void EntityModificationSystem::ApplyModification()
{
    ...
    if (transformChanged)
    {
        ...
        {
            sceneEditor->Exec(DAVA::Command::Create<TransformCommand>(modifEntities[i].object, modifEntities[i].originalTransform, modifEntities[i].object.GetLocalTransform()));
        }
        sceneEditor->EndBatch();
    }
}
    ...
    if (modifEntities.size() > 0 && clonedEntities.size() == modifEntities.size())
    {
        ...
        for (DAVA::uint32 i = 0; i < count; ++i)
        {
            ...
            if (cloneParent)
            {
                ...
                sceneEditor->Exec(DAVA::Command::Create<EntityAddCommand>(clonedEntities[i], cloneParent));
            }
...
void EntityModificationSystem::LockTransform(const SelectableGroup& entities, bool lock)
{
    ...
    {
        sceneEditor->Exec(DAVA::Command::Create<EntityLockCommand>(entity, lock));
    }
    sceneEditor->EndBatch();
}
...
void EntityModificationSystem::BakeGeometry(const SelectableGroup& entities, BakeMode mode)
{
    ...
    if (ro != nullptr)
    {
        ...
        if (entityList.size() > 0)
        {
            ...
            // bake render object
            sceneEditor->Exec(DAVA::Command::Create<BakeGeometryCommand>(ro, bakeTransform));
            ...
            for (it = entityList.begin(); it != entityList.end(); ++it)
            {
                ...
                sceneEditor->Exec(DAVA::Command::Create<TransformCommand>(Selectable(en), origTransform, newTransform));
                ...
                for (DAVA::int32 i = 0; i < en->GetChildrenCount(); ++i)
                {
                    ...
                    sceneEditor->Exec(DAVA::Command::Create<TransformCommand>(Selectable(childEntity), childOrigTransform, childNewTransform));
                }
            }
    ...
    else if (entity->GetChildrenCount() > 0) // just modify child entities
    {
        ...
        sceneEditor->Exec(DAVA::Command::Create<TransformCommand>(Selectable(entity), entity->GetLocalTransform(), entity->GetLocalTransform() * transform));
        ...
        for (DAVA::uint32 i = 0; i < count; ++i)
        {
            ...
            sceneEditor->Exec(DAVA::Command::Create<TransformCommand>(Selectable(childEntity), childEntity->GetLocalTransform(), childEntity->GetLocalTransform() * transform));
        }
...
void EntityModificationSystem::ApplyMoveValues(ST_Axis axis, const SelectableGroup& selection, const DAVA::Vector3& values, bool absoluteTransform)
{
    ...
    for (const Selectable& item : selection.GetContent())
    {
        ...
        sceneEditor->Exec(DAVA::Command::Create<TransformCommand>(item, origMatrix, newMatrix));
    }
    sceneEditor->EndBatch();
}
