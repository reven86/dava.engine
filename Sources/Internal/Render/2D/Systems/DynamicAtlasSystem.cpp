    ...
    if (sprite->inDynamicAtlas == false)
    {
        if (state == ACCUMULATING && IsValidPathAndType(sprite))
        {
            ...
            if (nullptr != region)
            {
                ...
                {
                    DVASSERT(std::find_if(regions.begin(), regions.end(), [&](std::shared_ptr<AtlasRegion>& region) {
...
void DynamicAtlasSystem::UnregisterSprite(Sprite* sprite)
{
    if (sprite->inDynamicAtlas)
    {
        ...
        if (sprite->inDynamicAtlas)
        {
            auto regionIt = std::find_if(regions.begin(), regions.end(), [&](std::shared_ptr<AtlasRegion>& region) {
                return region->sprite == sprite;
            });
    ...
    for (DynamicAtlas* atlas : atlasesCopy)
    {
        if (atlas != nullptr)
        {
            ...
            if (atlas->regions.empty())
            {
                ...
                auto ait = std::find_if(atlases.begin(), atlases.end(), [&](std::shared_ptr<DynamicAtlas>& atlasPtr) { return atlasPtr.get() == atlas; });
