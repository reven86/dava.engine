...
void ComponentManager::UpdateSceneComponentsSortedVector(const Type* type)
{
    ...
    // Keep sorted order
    auto position = std::lower_bound(sceneComponentsSortedByPermanentName.begin(), sceneComponentsSortedByPermanentName.end(), p, [this, db](const std::pair<uint32, uint32>& l, const std::pair<uint32, uint32>& r) {
